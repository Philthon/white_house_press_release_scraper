    ...
    for article_number in range(0, 10):
        try:
            header = driver.find_element(
                By.XPATH,
                "/html/body/section/div[2]/div/div/div[2]/div[2]/div/div[1]/div[{}]/div[2]/h3/a".format(
                    article_number + 1
                ),
            ).text
            date = driver.find_element(
                By.XPATH,
                "/html/body/section/div[2]/div/div/div[2]/div[2]/div/div[1]/div[{}]/div[1]/span".format(
                    article_number + 1
                ),
            ).text
            brief_format = "statements_and_releases"
...
def get_articles(driver, df, page_number) -> pd.DataFrame:
    for article_number in range(0, 10):
        try:
            ...
            brief_format = "statements_and_releases"
            date = driver.find_element(
                By.XPATH, '//*[@id="press_article_date_created"]'
            ).text
            header = driver.find_element(
                By.XPATH, "/html/body/section/div[2]/div/div/div[2]/div[2]/h1"
            ).text
            text = driver.find_element(
                By.XPATH, "/html/body/section/div[2]/div/div/div[2]/div[3]"
            ).text
            text = text.replace('"', "'")
